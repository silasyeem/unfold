...
For assembled captures, compare the source and render's signed screen directions: which way the long foot ends point, which panel edge is vertical, which face/rim is visible, and which existing versus newly introduced corner is elevated. A mirrored or opposite-side view is an orientation error even when the object is broadly side-lying and its parts are plausible. The main source drawing's viewing direction and page-up axis must be preserved; do not excuse a mismatch as an arbitrary camera angle. Propose corrected cameraDirection/cameraUp or physical corner assignment as appropriate. The player keeps its camera fixed and uses those axes to rotate the object.
