...
export async function requestStructured({apiKey,model,instructions,content,schema,name,maxTokens=10000,signal,fetchImpl=fetch}){
 const response=await fetchImpl('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model,store:false,instructions,input:[{role:'user',content}],text:{format:{type:'json_schema',name,strict:true,schema}},max_output_tokens:maxTokens,reasoning:{effort:'low'}}),signal});
