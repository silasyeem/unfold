...
Every Engine guide uses the shared player, including fresh PDF/photo conversions and reopened saved guides. The default camera stays fixed while step changes smoothly turn the build around its centre to the required working orientation. Playback and scrubbing keep that orientation steady. **Step view** offers a connection close-up on request; manually positioned cameras stay in place across steps. These transitions also apply to voice navigation and respect reduced-motion preferences.
