...
Every Engine guide uses the shared player, including fresh PDF/photo conversions and reopened saved guides. Step changes smoothly turn the build around its centre and move the camera directly to the next view. Playback and scrubbing keep the step's working orientation steady. These transitions also apply to voice navigation and respect reduced-motion preferences.
