...
3. Independently reads the complete manual, including cover and finished-product drawings, using high reasoning. This pass receives only page and step-number anchors from the extraction so earlier misread descriptions cannot bias its inventory. It identifies physical component types, quantities, and solid surfaces while preserving every extracted step's number, page, and order.
4. Generates compound primitive shapes and motion data with high reasoning, mapping every physical instance to its source component. The saved guide remains schema version 1; component evidence and coverage accompany it in the export envelope.
5. Checks structure, parenting, action timing, handling rotations, component quantities, broad surface geometry, and final visibility. An open perimeter cannot stand in for a solid tabletop or shelf. Permanent parts must remain present and temporary tools must be removed. One correction attempt is allowed before returning a recoverable error.
6. Plays the validated draft in Three.js. Attached hardware follows its parent part. Seeking computes state from the guide, so direct jumps and replay agree.
...
| `dist/screw-geometry.js`, `dist/screw-viewer.js`, `dist/screws.js` | Parametric threaded boundary mesh, shared 3D preview, dimension validation and browser STL export. |
...
- Requests with a supplied cross-origin `Origin` are rejected. Two conversions can run concurrently in each process; each has a twenty-minute timeout. This is a local/private deployment boundary, not public authentication or account rate limiting.
...
The existing `.openai/hosting.json` refers to a static Sites deployment. The prepared guide and Screw lab can run as static pages. Static hosting alone cannot run conversion or parts scanning, so those APIs remain local. `server/worker.mjs` is a source entry, not a packaged deployment: it still needs bundling, Node compatibility for the current library modules, a durable library adapter, an `ASSETS` binding, a server secret, and access control before publishing. The local Node server is the supported engine runtime in this version.
