...
export function mountManualLibrary(container,{onPdfReady=()=>{},onError=()=>{},onBusy=()=>{},onDemo=null}={}){
 ...
 const demo=element('button','button primary library-demo','Open KNARREVIK demo →');demo.type='button';demo.hidden=true;
 ...
 dialog.append(heading,intro,form,status,demo,results,webArea);container.append(launcher,dialog);
 let controller=null,serial=0,busy=false,externalDisabled=false,lastQuery='',lastData=null;
 const report=(message,demoAvailable=false)=>{status.textContent=message;status.classList.add('error');demo.hidden=!demoAvailable||!onDemo;onError(message);};
 function setBusy(value){busy=value;submit.disabled=value;input.disabled=value;demo.disabled=value;web.disabled=value||lastData?.webSearchAvailable===false;launcher.disabled=externalDisabled||value;results.querySelectorAll('button').forEach(button=>{button.disabled=value;});dialog.setAttribute('aria-busy',String(value));onBusy(value);}
 const api=async(url,options={})=>{const response=await fetch(url,options);if(!response.ok){const data=await response.json().catch(()=>({}));const error=new Error(data.error||'The manual library is unavailable. Start the Unfold server and try again.');error.demoAvailable=data.demoAvailable===true;throw error;}return response;};
 function render(data){
  lastData=data;results.replaceChildren();status.classList.remove('error');demo.hidden=true;
 ...
 async function search(online=false){
  ...
  const current=++serial;controller?.abort();controller=new AbortController();lastQuery=query;setBusy(true);demo.hidden=true;status.classList.remove('error');status.textContent=online?'Searching manufacturer instructions…':'Looking for matching manuals…';
  try{
  ...
  }catch(error){if(current===serial&&error.name!=='AbortError')report(error.message,error.demoAvailable);}
 ...
 demo.onclick=()=>{if(busy||externalDisabled)return;dialog.close();return onDemo?.();};
