...
import {assertGuide,MAX_GUIDE_BYTES} from './guide-schema.js';
...
import {mountConversionProgress} from './conversion-progress.js';
const $=s=>document.querySelector(s);
const conversionProgress=mountConversionProgress($('#conversion-progress'));
const labels={upside_down:'Upside down',upright:'Upright',on_back:'On its back',on_front:'On its front',on_left:'On its left side',on_right:'On its right side'};
...
function syncVoice(){
 const available=voiceAvailable(),hasGuide=Boolean(output)&&!choosingManual;
 $('#copilot-toggle').hidden=!hasGuide;$('#screw-lab').hidden=!hasGuide;
 if(!hasGuide){$('#copilot-panel').hidden=true;$('#copilot-toggle').setAttribute('aria-expanded','false');}
 copilot?.refreshAvailability();if(!available&&copilot?.current)copilot.end();
}
...
function setStep(next){
 ...
 if(pdf){linked=true;showPage(step?.sourcePage||1);}syncReviewState();
}
...
$('#convert').onclick=async()=>{
 ...
 try{if(hostedRendering){const {convertInBrowser}=await import('./hosted-convert.js');const result=await convertInBrowser(file,{pageCount:pdf.numPages,signal:current.signal,onStage:message=>status(message,{busy:true}),onContact:()=>conversionProgress.contact()});if(result.provenance?.sha256!==pdfHash)throw new Error('The generated guide does not match the uploaded PDF.');loadGuide(result);status('Your 3D draft is ready. Review it against the original manual.');return;}const response=await fetch('/api/convert',{method:'POST',headers:{'Content-Type':'application/pdf','X-Unfold-Convert':'1','X-Pdf-Pages':String(pdf.numPages),'X-Pdf-Name':encodeURIComponent(file.name)},body:file,signal:current.signal});conversionProgress.contact();if(!response.ok){const error=await response.json().catch(()=>({}));throw new Error(error.error||'Conversion is unavailable. Start the Unfold server and try again.');}
...
addEventListener('pagehide',()=>{conversionProgress.stop();conversion?.abort();demoController?.abort();cancelAnimationFrame(animation);viewer?.dispose();guideScanner.destroy();photoIntake?.destroy();manualLibrary?.destroy();pdf?.destroy();},{once:true});
