...
let photoIntake,manualLibrary,conversionBusy=false;const intakeBusy={photos:false,library:false};
...
function syncBusy(){const busy=conversionBusy||intakeBusy.photos||intakeBusy.library;$('#cancel').hidden=!conversionBusy;$('#convert').disabled=busy||!file||!pdf;for(const input of [$('#pdf-file'),$('#empty-pdf-file'),$('#guide-file')])input.disabled=busy;photoIntake?.setDisabled(busy);manualLibrary?.setDisabled(busy);}
function setBusy(value){conversionBusy=value;syncBusy();}
...
function loadGuide(result){
 ...
 for(const selector of ['#export','#play','#speed','#step-view','#whole-view','#exploded'])$(selector).disabled=false;setStep(-1);
}
async function pickPdf(selected){
 ...
 let candidate;
 try{
  status('Reading the PDF…');const bytes=new Uint8Array(await selected.arrayBuffer());if(!new TextDecoder().decode(bytes.subarray(0,1024)).includes('%PDF-'))throw new Error('Choose a valid PDF file.');
...
$('#guide-file').onchange=async e=>{const selected=e.target.files[0];if(!selected)return;const serial=++uploadId;conversion?.abort();try{if(selected.size>2*1024*1024)throw new Error('Guide files must be smaller than 2 MB.');const result=JSON.parse(await selected.text());assertGuide(result.guide);if(serial!==uploadId)return;renderId++;renderTask?.cancel();await pdf?.destroy();if(serial!==uploadId)return;pdf=null;file=null;pdfHash=null;$('#pdf-canvas').hidden=true;$('#manual-empty').hidden=false;$('#manual-empty').textContent='Upload the original PDF to relink its source diagrams.';$('#manual-link-state').textContent='No PDF';$('#page-label').textContent='—';for(const selector of ['#enlarge','#page-prev','#page-next','#convert'])$(selector).disabled=true;$('#relink').hidden=true;loadGuide(result);$('#conversion-title').textContent='Saved guide opened';status('Upload its original PDF to view linked diagrams.');}catch(error){status(error.message||'This guide could not be opened.',{error:true});}};
$('#export').onclick=()=>{if(!output)return;const blob=new Blob([JSON.stringify({...output,reviewedSteps:[...reviewed]},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=(output.guide.productName.replace(/[^a-zA-Z0-9_-]/g,'_')||'assembly')+'.unfold.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
...
document.addEventListener('keydown',e=>{if(!output||document.querySelector('dialog[open]')||e.target.closest('input,textarea,select,button,a'))return;if(e.code==='Space'){e.preventDefault();$('#play').click();}if(e.code==='ArrowRight'){e.preventDefault();$('#next').click();}if(e.code==='ArrowLeft'){e.preventDefault();$('#prev').click();}});
...
photoIntake=mountPhotoIntake($('#photo-intake'),{onPdfReady:pickPdf,onError:intakeError,onBusy:value=>{intakeBusy.photos=value;syncBusy();}});
manualLibrary=mountManualLibrary($('#manual-library'),{onPdfReady:pickPdf,onError:intakeError,onBusy:value=>{intakeBusy.library=value;syncBusy();}});
syncBusy();
