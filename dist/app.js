...
const modelContext=document.modelContext;if(modelContext?.registerTool){const lifecycle=new AbortController();const register=tool=>{try{Promise.resolve(modelContext.registerTool(tool,{signal:lifecycle.signal})).catch(()=>{});}catch{}};
 register({name:'get_assembly_state',description:'Read the current prepared STRANDMON guide step and playback state.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>({product:'STRANDMON',step,title:steps[step].title,progress,playing,manualPage,preparedGuide:true})});
 register({name:'navigate_assembly_step',description:'Navigate to one of the 16 prepared STRANDMON assembly steps; 0 opens the overview.',inputSchema:{type:'object',properties:{step:{type:'integer',minimum:0,maximum:16}},required:['step'],additionalProperties:false},annotations:{readOnlyHint:false},execute:async input=>{if(!input||Object.keys(input).some(k=>k!=='step')||!Number.isInteger(input.step)||input.step<0||input.step>16)throw new Error('step must be an integer from 0 to 16');await setStep(input.step);return{step,title:steps[step].title,manualPage,preparedGuidePage:steps[step].page};}});addEventListener('pagehide',()=>lifecycle.abort(),{once:true});}
