...
export default {async fetch(request,env,ctx){
 ...
 if(['/api/convert','/api/photos','/api/library'].includes(path)||path.startsWith('/api/library/'))return Response.json({error:'Manual conversion and search run in the local workspace. You can open the saved KNARREVIK demo here.'},{status:503,headers:{'Cache-Control':'no-store'}});
