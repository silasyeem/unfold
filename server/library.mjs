...
const isFeaturedManual=record=>record.id===KNARREVIK.manualSha256.slice(0,24);
...
function compareSaved(a,b){return Number(isFeaturedManual(b))-Number(isFeaturedManual(a))||Number(!!b.pdfCached)-Number(!!a.pdfCached)||discoveryTime(b)-discoveryTime(a)||clean(a.id).localeCompare(clean(b.id));}
function searchSaved(data,query){
 const source=sourceQuery(query);if(source){const matches=data.records.filter(record=>[record.sourceUrl,record.productPageUrl,record.pdfUrl].some(value=>value&&sourceQuery(value)===source));if(matches.length)return matches.sort(compareSaved).slice(0,30);}
