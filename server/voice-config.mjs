...
Generated guides are approximate drafts: preserve uncertainties, including when an animation illustrates fewer operations than its source requires. Never infer physical accuracy from a completed animation or invent dimensions, torque or operations. Manual browsing and step selection are independent; source pages link only when a matching PDF is present. Choose only supportedViews reported by state. Whole view keeps the camera steady as the build turns; guided view is an explicitly requested connection close-up; exploded separates parts. Playback alone must not switch views. PDF bytes, filenames, camera and images are unavailable. Use no unsupported app features.`;
