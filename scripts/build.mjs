...
for (const entry of await readdir(dist, {withFileTypes: true})) {
  ...
  if (entry.isDirectory() && !['assets', 'vendor'].includes(entry.name)) throw new Error(`Review public directory before packaging: ${entry.name}`);
...
await build({entryPoints: [join(root, 'server/hosted.mjs')], outfile: join(dist, 'server/index.js'), bundle: true, format: 'esm', platform: 'browser', target: 'es2022', sourcemap: false});
